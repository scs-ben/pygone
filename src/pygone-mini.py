...
def N(O):return chr(O+96)
def Q(P):print(P,flush=1)
def R(S,K7,U,V,W,X):Q(f"info depth {S} score cp {K7} time {U} nodes {V} nps {W} pv {X}")
def Y(A1):return O7(A1[0:2]),O7(A1[2:4])
def O8(O0):return N(O0%10)+str(abs(O0//10-10))
def O7(Z7):return 10*(abs(int(Z7[1])-8)+2)+(ord(Z7[0])-97)+1
class A6:
	...
	def B8(Z,A1):
		l_A7=Z.A7;i=Z.A8%2==0;Z.M9+=1;B0,O=Y(A1);C2=l_A7[B0].lower()
		if l_A7[O]!='-':Z.M9=0
	...
	def C4(Z,A1):
		...
		if'e8'in A1:
			...
			if u.B3[0]:u.ZY^=u.ZV[2];u.B3[0]=0
		elif'h8'in A1:
			if u.B3[1]:u.ZY^=u.ZV[3];u.B3[1]=0
	...
	def A4(Z,A1,M4=0):
		i=Z.A8%2==0;l_A7=Z.A7;B0,O=Y(A1);D1=0 if i else 119;to_D1=abs(O-D1);from_D1=abs(B0-D1);d=0;C2=l_A7[B0].lower();D4=l_A7[O].lower()
		...
		return d
	def ZH(Z,O0):
		i=Z.A8%2==0;D11=-10 if i else 10;ZN=O0+D11;K4=1
		while 20<=ZN<=100:
			if not Z.A7[ZN]in'-.':return 0
			ZN+=D11
		return 1
	def ZM(Z,O0):
		i=Z.A8%2==0;D11=-10 if i else 10;D5='P'if i else'p';ZN=O0+D11
		while 20<=ZN<=100:
			if Z.A7[ZN]==D5:return 1
			ZN+=D11
		return 0
	...
	def A5(Z,N8=0):
		...
		for(O0,Z2)in enumerate(Z.A7):
			...
			E9=O8(O0);K0=Z2.lower()
			if Z2=='p':D1=-1
			if not N8:
				if Z2=='K':
					if Z.B2[1]and Z.A7[96:99]=='--R'and not any(Z.O2(i,Z7)for Z7 in['e1','f1','g1']):yield E9+'g1'
					if Z.B2[0]and Z.A7[91:95]=='R---'and not any(Z.O2(i,Z7)for Z7 in['e1','d1','c1']):yield E9+'c1'
				elif Z2=='k':
					if Z.B3[1]and Z.A7[26:29]=='--r'and not any(Z.O2(i,Z7)for Z7 in['e8','f8','g8']):yield E9+'g8'
					if Z.B3[0]and Z.A7[21:25]=='r---'and not any(Z.O2(i,Z7)for Z7 in['e8','d8','c8']):yield E9+'c8'
				elif K0=='p'and E7<=O0<E7+8 and Z.A7[O0+-10*D1]=='-'and Z.A7[O0+-20*D1]=='-':yield E9+O8(O0+-20*D1)
			for F3 in I93(K0):
				P3=O0+F3[0]+F3[1]*D1
				while 20<P3<99:
					F6=Z.A7[P3]
	...
	def O2(Z,i,Z7):
		D1=1;E5='PRNBQK-'if i else'prnbqk-';O2=O7(Z7)
		for(O0,Z2)in enumerate(Z.A7):
		...
		return 0
class F0:
	V=0;S=0;G1=0;G2=0;G3={};ZA=1;ZB=2;ZC=3;ZD=A['k']
	...
	def H1(Z,l):
		...
		for S in range(1,100):
			...
			R(str(S),str(math.ceil(d)),str(math.ceil(H8*1000)),str(Z.V),str(W),str(H7+pv));yield(S,H7,d)
	def H9(Z,l,S,a,b):
